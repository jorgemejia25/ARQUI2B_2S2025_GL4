...
inline void readButtonsAndTrigger(){
  ...
  if (b1 != btn1Last){ btn1Last = b1; btn1ChangeMs = now; }
    ...
    panicButtonsActiveState[0] = true; panicButtonsUntil[0] = now + 3000;
    buzzPin(BUZ_TU1_PIN);
  ...
  if (b2 != btn2Last){ btn2Last = b2; btn2ChangeMs = now; }
    ...
    panicButtonsActiveState[1] = true; panicButtonsUntil[1] = now + 3000;
    buzzPin(BUZ_TU2_PIN);
  ...
  if (b3 != btn3Last){ btn3Last = b3; btn3ChangeMs = now; }
    ...
    panicButtonsActiveState[2] = true; panicButtonsUntil[2] = now + 3000;
    buzzPin(BUZ3_PIN);
  ...
  if (b4 != btn4Last){ btn4Last = b4; btn4ChangeMs = now; }
    ...
    panicButtonsActiveState[3] = true; panicButtonsUntil[3] = now + 3000;
    buzzPin(BUZ4_PIN);
...
const char* stateForIndex(int idx){
  ...
  if (g == SEM_A){
    if (phase == P_A_GREEN)  return "VERDE";
    if (phase == P_A_YELLOW) return "AMARILLO";
    return "ROJO";
  } else {
    if (phase == P_B_GREEN)  return "VERDE";
    if (phase == P_B_YELLOW) return "AMARILLO";
    return "ROJO";
  }
}
...
// =====================================================
enum TuDir : uint8_t { TU_UNKNOWN=0, TU_1_TO_2=12, TU_2_TO_1=21 };
...
inline void processSD(uint8_t idx){
  ...
  if (below){
    ...
    if (!sdAlerted[idx] && sdConsec[idx] >= SD_REQ){
      ...
      Serial.print(" -> Semaforo S"); Serial.print(sIdx);
...
void emitStatusJson(){
  ...
  Serial.print("\"gas_ppm\":{\"Z1\":"); Serial.print(gas1);
  Serial.print(",\"Z2\":"); Serial.print(gas2); Serial.print("},");

  Serial.print("\"zumbador\":{\"Z1\":0,\"Z2\":0},");

  Serial.print("\"sismo\":{\"activo\":0,\"magnitud\":0.0,\"origen\":\"\"},");

  Serial.print("\"panic_buttons\":{");
  for (int i=0;i<4;i++){
    Serial.print("\"PB"); Serial.print(i+1); Serial.print("\":");
    Serial.print("{\"activo\":"); Serial.print(panicButtonsActiveState[i]?1:0);
  ...
  for (int sd = 0; sd < 5; sd++){
    if (sdAlerted[sd]){
      ...
      Serial.print('"'); Serial.print('S'); Serial.print(sIdx); Serial.print('"');
      first=false;
    }
  }
  Serial.print("],");
...
void loop() {
  for (int i=0;i<4;i++)
    if (panicButtonsActiveState[i] && millis() >= panicButtonsUntil[i])
